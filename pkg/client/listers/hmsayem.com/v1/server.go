...
type ServerLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Server, err error)
...
// List lists all Servers in the indexer.
func (s *serverLister) List(selector labels.Selector) (ret []*v1.Server, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Server))
	})
	return ret, err
}
...
// List lists all Servers in the indexer for a given namespace.
func (s serverNamespaceLister) List(selector labels.Selector) (ret []*v1.Server, err error) {
...
// Get retrieves the Server from the indexer for a given namespace and name.
func (s serverNamespaceLister) Get(name string) (*v1.Server, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("server"), name)
	}
	return obj.(*v1.Server), nil
}
